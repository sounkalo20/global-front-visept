...
import { User, Phone, Calendar, Receipt, Utensils, Printer } from 'lucide-react';
                ...
                <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <div className="flex items-center justify-between pr-6">
                            <DialogTitle>{loading ? 'Chargement...' : `Dette ${debt?.sale_number || ''}`}</DialogTitle>
                            {debt && (
                                <Button
                                    variant="outline"
                                    size="sm"
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-4">
                            ...
                            {(() => {
                                ...
                                if (!discAmt && itemsSubtotal > Number(debt.total_amount)) {
                                    discAmt = itemsSubtotal - Number(debt.total_amount);
                                }
                                const discType = debt.sale_discount_type || debt.discount_type || (discAmt > 0 ? 'fixed' : 'none');
                                const discVal = debt.sale_discount_value || debt.discount_value || (discType === 'percentage' ? Math.round((discAmt / (itemsSubtotal || Number(debt.total_amount) + discAmt)) * 100) : discAmt);
                                const subTot = Number(debt.sale_subtotal || debt.subtotal || itemsSubtotal || Number(debt.total_amount) + discAmt);
                                if (discAmt <= 0) return null;
                                return (
                                    <div className="bg-pink-50 border border-pink-100 rounded-xl p-3 flex justify-between items-center text-xs">
                                        <div>
                                            <span className="text-gray-500">Sous-total HT : </span>
                                            <span className="font-semibold text-gray-700">{subTot.toLocaleString()} FCFA</span>
                                        </div>
                                        <div>
                                            <span className="text-[#d4006e] font-bold">
                                                Remise ({discType === 'percentage' ? `${discVal}%` : 'Fixe'}) : -{discAmt.toLocaleString()} FCFA
                                            </span>
                                        </div>
                                    </div>
                                );
