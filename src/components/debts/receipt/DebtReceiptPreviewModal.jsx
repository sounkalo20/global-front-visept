...
import DebtReceiptTemplate from './DebtReceiptTemplate';
import DebtInvoiceA4Template from './DebtInvoiceA4Template';
import useDebtReceiptPrinter from '@/hooks/useDebtReceiptPrinter';
import useAuthStore from '@/store/authStore';
import useCompanyStore from '@/store/companyStore';
import { debtsApi } from '@/lib/api/debts';

export default function DebtReceiptPreviewModal({ debt, open, onOpenChange, onClosed }) {
  const [paperSize, setPaperSize] = useState('A4');
  const [fullDebt, setFullDebt] = useState(debt);
  const [loading, setLoading] = useState(false);

  const { printDebtReceipt, isPrinting } = useDebtReceiptPrinter();
  ...
  useEffect(() => {
    if (open && debt) {
      setFullDebt(debt);
      // Si les paiements ne sont pas déjà chargés sur l'objet debt, on va les chercher
      if (debt.id && activeCompany?.id && (!debt.payments || !debt.sale_items)) {
        setLoading(true);
        debtsApi.getById(debt.id, activeCompany.id)
          .then((res) => {
            const fetchedDebt = res.data?.data?.debt;
            if (fetchedDebt) {
              setFullDebt(fetchedDebt);
            }
          })
          .catch((err) => {
            console.error('Erreur lors du chargement des détails de la dette:', err);
          })
          .finally(() => setLoading(false));
      }
    }
  }, [debt, open, activeCompany]);

  if (!debt) return null;
  const debtToRender = fullDebt || debt;

  const handlePrint = () => {
    printDebtReceipt(debtToRender, activeCompany, user, paperSize);
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-3xl md:max-w-4xl w-full max-h-[95vh] overflow-hidden flex flex-col bg-gray-100 p-0">
        <DialogHeader className="bg-white px-6 py-4 border-b shrink-0">
          <DialogTitle>Aperçu avant impression de la dette</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex items-center gap-3 px-6 py-3 bg-white border-b shrink-0">
          ...
          <span className="text-xs text-gray-400 ml-2">
            {isA4 ? 'Fiche A4 avec historique des paiements' : `Ticket thermique ${paperSize}`}
          </span>
        </div>
          ...
          {loading ? (
            ...
            >
              ...
              <div className="a4-preview">
                <DebtInvoiceA4Template debt={debtToRender} company={activeCompany} user={user} />
              </div>
            </div>
          ) : (
            ...
            >
              ...
              <div className="receipt-preview">
                <DebtReceiptTemplate debt={debtToRender} company={activeCompany} user={user} />
              </div>
            </div>
          )}
        </div>
